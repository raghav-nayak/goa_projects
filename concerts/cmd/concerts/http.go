...
import (
	...
	concertssvr "concerts/gen/http/concerts/server"
	"context"
	"net/http"
	"net/url"
	"sync"
...
func handleHTTPServer(ctx context.Context, u *url.URL, concertsEndpoints *concerts.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		concertsServer *concertssvr.Server
	)
	{
		eh := errorHandler(ctx)
		concertsServer = concertssvr.New(concertsEndpoints, mux, dec, enc, eh, nil)
	}

	// Configure the mux.
	concertssvr.Mount(mux, concertsServer)
	...
	for _, m := range concertsServer.Mounts {
