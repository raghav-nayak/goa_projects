...
var _ = API("concerts", func() {
    ...
    Server("concerts", func() {
        Description("Convert management server")
        Host("localhost", func ()  {
...
var _ = Service("concerts", func(){
    Description("The concerts services manages music concert data. It provide CRUD operations for concert information including artist details, venues, dates, and pricing.")
    ...
    Method("list", func ()  {
        ...
        Payload(func ()  {
            ...
            Attribute("limit", Int, "Num of items per page", func() {
                Minimum(1)
                Maximum(100)
                Default(10)
                Example(10)
    ...
    Method("show", func() {
		...
		Payload(func() {
			Attribute("concertID", String, "Unique concert identifier", func() {
				Format(FormatUUID) // UUID format for unique identification
...
// Base type with all concert fields, no required constraints
var ConcertData = Type("ConcertData", func() {
	...
	Attribute("date", String, "Concert date in ISO 8601 format (YYYY-MM-DD)", func() {
		Format(FormatDate) // ISO 8601 format (YYYY-MM-DD) validation
		Example("2024-12-25")
...
// For creation - extends base type and requires all fields
...
// For responses - complete concert with ID and all details
